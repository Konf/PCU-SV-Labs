...
\par{Элемент памяти — один из самых важных элементов цифровых устройств. Чтобы не делать ошибок при разработке цифровых устройств, необходимо понять место этого узла, его идею и инструменты языка \eng{System Verilog}, связанные с ним.}
...
\par{Опишем защелку на языке \eng{System Verilog}, опираясь на её структуру, которую мы рассмотрели выше. Нам понадобятся два входа, два выхода и два элемента И-НЕ, которые мы опишем с помощью операций И (оператор \&) и НЕ (оператор \textasciitilde).}

%\lstinputlisting[caption={Описание \eng{RS}-защелки на языке \eng{System Verilog}}, ]{./code_examples/lab_2/rs_latch.v}

\begin{listing}[H]
	\inputminted{SystemVerilog}{code_examples/lab_2/rs_latch.sv}
	\caption{Описание \eng{RS}-защелки на языке \eng{System Verilog}}
	%\label{lab2:code1}
\end{listing}
...
\begin{listing}[H]
	...
	\caption{Поведенческое описание \eng{D}-защелки на языке \eng{System Verilog}}
	%\label{lab2:code1}
\end{listing}
...
%\lstinputlisting[caption={Описание \eng{D}-триггера на языке \eng{System Verilog}}, ]{./code_examples/lab_2/d_flipflop.v}

\begin{listing}[H]
	\inputminted{SystemVerilog}{code_examples/lab_2/d_flipflop.sv}
	\caption{Описание \eng{D}-триггера на языке \eng{System Verilog}}
	%\label{lab2:code1}
\end{listing}
...
%\lstinputlisting[caption={Описание регистра на языке \eng{System Verilog}}, ]{./code_examples/lab_2/register.v}
\begin{listing}[H]
	\inputminted{SystemVerilog}{code_examples/lab_2/register.sv}
	\caption{Описание регистра на языке \eng{System Verilog}}
	%\label{lab2:code1}
\end{listing}
...
\par{Запомните описание регистра. Оно используется при проектировании практически любого цифрового устройства с помощью \eng{System Verilog}.}

\par{Необходимо отметить важную концепцию языка \eng{System Verilog}. \textbf{Один сигнал может быть изменен (записан) только в пределах одного блока \eng{always}. Сигналы доступны для проверки в любом из блоков, но изменять их значение можно только в одном из них.}}

%\lstinputlisting[caption={Пример присвоения значения переменной в разных блоках \eng{always} на языке \eng{System Verilog}}, ]{./code_examples/lab_2/error_example.v}
\begin{listing}[H]
	\inputminted{SystemVerilog}{code_examples/lab_2/error_example.sv}
	\caption{Пример присвоения значения переменной в разных блоках \eng{always} на языке \eng{System Verilog}}
	%\label{lab2:code1}
\end{listing}
...
\par{Опишем поведение такого счётчика на \eng{System Verilog}.}

%\lstinputlisting[caption={Описание восьмибитного счетчика на языке \eng{System Verilog}}, ]{./code_examples/lab_2/counter.v}
\begin{listing}[H]
	\inputminted{SystemVerilog}{code_examples/lab_2/counter.sv}
	\caption{Описание восьмибитного счетчика на языке \eng{System Verilog}}
	%\label{lab2:code1}
\end{listing}
...
%\lstinputlisting[caption={Описание схемы синхронизации на языке \eng{System Verilog}}, ]{./code_examples/lab_2/button_syncroniser.v}
\begin{listing}[H]
	\inputminted{SystemVerilog}{code_examples/lab_2/button_syncroniser.sv}
	\caption{Описание схемы синхронизации на языке \eng{System Verilog}}
	%\label{lab2:code1}
\end{listing}
...
\par{Описать на языке \eng{System Verilog} цифровое устройство, функционирующее согласно следующим принципам:
...
\par{Выполнив описание модуля на языке \eng{System Verilog}, необходимо построить временные диаграммы его работы с помощью САПР \eng{Altera Quartus}.}
